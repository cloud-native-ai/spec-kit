...
} from "../../scripts/js/better-harness/agent-lint/index.mjs";
...
test("agent-lint CLI emits parser-safe JSON through the Better Harness facade", async () => {
  const root = await makeFixture();
  const cliPath = path.join(process.cwd(), "scripts", "js", "better-harness", "agent-lint", "cli.mjs");
...
// spec-kit subset: "agents-md-review evidence-backed findings" and "findings recommendations catalog"
// tests removed — they assert the findings-recommend verdict-layer catalog, which is excluded
// from the collection subset per UPSTREAM.md / contracts C-B2.
