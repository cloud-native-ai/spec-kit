...
Orchestrate a **multi-Agent team** forming a self-iterating closed-loop system with Supervisor (strategy), Meta-Coordinator (coordination), and Workers (execution).
...
3. **Configure Meta-Coordinator**: Task decomposition strategy, dispatch pattern, team roster
4. **Configure Supervisor**: Quality dimensions + weights, threshold (default: 0.8), max iterations (default: 5), regression limit (default: 2)
...
LOOP (iteration in 1..max_iterations):
  ...
  PHASE 1 — COORDINATE:
    Meta-Coordinator decomposes goal, assigns sub-tasks, selects dispatch strategy

  PHASE 2 — EXECUTE:
    Workers execute assigned sub-tasks, write deliverables
    Meta-Coordinator monitors, handles failures, consolidates results

  PHASE 3 — EVALUATE:
    Supervisor scores output on each quality dimension
  ...
  PHASE 5 — IMPROVE (if continuing):
    Supervisor generates improvement feedback
    Meta-Coordinator adjusts strategy, triggers improve-agent on weak areas
```
