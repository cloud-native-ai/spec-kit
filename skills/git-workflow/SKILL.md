...
| 输入 | 作用域 | 行为 |
|------|--------|------|
| 配置文档 `.specify/memory/git-workflow.md` 不存在 | **Bootstrap** | 交互确认分支名、创建分支、初始化 `.gitexcludes`、生成配置文档 |
| 文档存在 且 无操作参数 | **全维度体检** | R1 观测 + R3 diff + 残差汇报，不自动收敛 |
| 文档存在 且 有操作参数 | **定向收敛** | 用户指令并入期望态，执行具体 git 操作（自动尊重 `.gitexcludes`） |
...
1. 若旧位置 `docs/git-workflow.md` 存在而 `.specify/memory/git-workflow.md` 不存在，先迁移（命令见 [bootstrap-commands.md](./references/bootstrap-commands.md#旧位置文档迁移)）。
2. 检查 `${SKILL_WORKDIR}/.specify/memory/git-workflow.md` 是否存在。
...
当 `${SKILL_WORKDIR}/.specify/memory/git-workflow.md` 不存在时进入：期望态 = 完整三层工作流骨架，通过最少必要问题（逐一确认，不臆造分支名）+ 自动探测补齐信息。命令序列见 [bootstrap-commands.md](./references/bootstrap-commands.md)。
...
在归口 instructions 文档的 Documentation Map 中添加 Git Workflow 引用行。查找优先级与写入内容见 [instructions-lookup.md](./references/instructions-lookup.md)。

---

### 全维度体检（Maintain）— 观测 + diff + 残差汇报

当 `${SKILL_WORKDIR}/.specify/memory/git-workflow.md` 存在且用户未传入操作参数时进入。只观测与 diff，不自动收敛；收敛建议列入报告尾部待用户确认。命令序列见 [health-check-commands.md](./references/health-check-commands.md)。

#### 2.1 加载配置

从 `${SKILL_WORKDIR}/.specify/memory/git-workflow.md` frontmatter 读取分支映射：`MAIN` = `main_branch`、`PRE` = `pre_branch`、`DEV` = `dev_branch`。
...
当 `${SKILL_WORKDIR}/.specify/memory/git-workflow.md` 存在且用户传入了具体操作参数时进入：用户指令并入期望态，按下列预定义操作收敛，写远端/直接合入类动作遵循 Overview 的分级确认门禁。命令序列见 [execute-commands.md](./references/execute-commands.md)。
...
- **更新**：分支改名时更新 frontmatter 映射；新增异常经验追加到 [troubleshooting.md](./references/troubleshooting.md)；更新 `last_updated` 日期。
