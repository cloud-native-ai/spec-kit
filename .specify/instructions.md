...
| git-workflow | <SKILL:.specify/skills/git-workflow/SKILL.md> | Three-tier Git workflow management (trunk/pre-release/dev). Three modes: Setup (establish workflow), Maintain (health check), Execute (run git operations). Uses .specify/memory/git-workflow.md as single source of truth. | - | true | false | .specify/skills/git-workflow/SKILL.md |
