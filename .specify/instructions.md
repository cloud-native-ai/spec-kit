...
- **A green check is not proof (blind-check class)**: a verification command can return an empty set, `EXIT=0`, or a passing test for a reason unrelated to the proposition it was written to judge. This is the worst defect shape in this repo — red gets fixed, green gets cited as evidence downstream. After writing any assertion about *absence* (nothing changed, nothing dangles, nothing leaked, no new files), answer first: **would this go red if the guarded thing were actually broken?** For a guard of a negative proposition the evidence is a **mutation drill** — break the subject, confirm red, restore, confirm green, delete the probe artifact and verify it is gone — not a passing run on the current tree. Pair every "filtered set is empty" assertion with an **anti-vacuity sentinel** asserting some companion quantity is non-empty, so "empty because correct" stays distinguishable from "empty because blind". Full class with its four measured causes and both techniques: `docs/reference/history/00-cross-cutting-lessons.md` § 十二.
...
- **Environment & tooling gotchas**:
  ...
  - **`git diff` forms for "what did this run change"**: `--stat` defeats any path regex (long paths are elided to `.../name` and the line ends in a change histogram), `git diff HEAD` is unconditionally true under a clean CI checkout, and `git diff <sha>` reaches **tracked** paths only — so files added but not yet committed are invisible to it, which is exactly the window a mid-run gate cares about. Use `--name-only` against a frozen literal SHA, unioned with `git ls-files --others --exclude-standard`, and never substitute an extension filter for a path filter. Detail and the `&&`-chain exit-code trap: `docs/reference/history/00-cross-cutting-lessons.md` § 十三.
  ...
  - **`.specify/.specify/` stray projection (same trap, two more variants)**: invoking `.specify/scripts/python/<engine>.py` whose root is `parents[2]` mis-roots to `.specify` and nests every mirror target one level deep; likewise `specify init --here` from inside `.specify/` installs a framework tree into the runtime dir. Guard: a dir named `.specify` is never a root (sync-mirrors/gate-check/tools-utils correct upward; CLI render/init reject outright); drift guard `tests/contract/test_mirror_root_guard.py`. Rule owner: `.specify/shared/definitions/dogfooding-definitions.md` § 2.1 rule 4.
