...
| manage-project | <SKILL:.specify/skills/manage-project/SKILL.md> | Lightweight project management (evolved from summarize-project): one Markdown management document (default .specify/project/project.md) as the single source of truth covering background, milestones, main work (WBS), and progress tracking (Gantt). Charts are embedded as editable PlantUML source blocks; rendering validation is delegated to draw-plantuml; repeat runs update incrementally. | - | true | false | .specify/skills/manage-project/SKILL.md |
