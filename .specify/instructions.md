## Project Overview
**Spec Kit (spec-kit)** is an SDD-oriented CLI toolkit: it uses templates and scripts to generate `.specify/` structure and instruction files, driving the `/speckit.*` spec → plan → tasks → implement workflow, and supports Claude Code, Codex CLI, Qoder CLI, GitHub Copilot, opencode, Qwen Code, Hermes Agent, and iFlow CLI.
...
| **Feature Index** | `.specify/memory/features.md` | Feature roadmap status | 35 features tracking /speckit.* commands, AI tool support (Claude/Codex/Qoder/Copilot/opencode/Qwen/Hermes/iFlow), skills, and core capabilities |
...
- **Key Directories**:
  ...
  - `skills/`: Installed Spec Kit skills (22 total: analysis-project, browser-utils, cli-setup, clone-website-ui, create-agent, create-skills, create-team, database-utils, document-utils, draw-d3js, draw-echarts, draw-plantuml, extension-e2e-test, git-submodule-edit, git-workflow, improve-agent, improve-skills, improve-team, manage-project, memory-recall, memory-record, think-skills). Mirrored to `.specify/skills/` via package install; `.github/skills/` is a compatibility symlink.
...
| manage-project | <SKILL:.specify/skills/manage-project/SKILL.md> | Lightweight project management (evolved from summarize-project): one Markdown management document (default .specify/project/project.md) as the single source of truth covering background, milestones, main work (WBS), progress tracking (Gantt), and an agile management loop (requirement→feature→task/staffing→testcase→evaluation, iterated) whose stages anchor to existing spec-kit artifacts; reserved integration extension points (Jira/CI-CD/SCM, not implemented). Charts are embedded as editable PlantUML source blocks; rendering validation is delegated to draw-plantuml; repeat runs update incrementally. | - | true | false | .specify/skills/manage-project/SKILL.md |
