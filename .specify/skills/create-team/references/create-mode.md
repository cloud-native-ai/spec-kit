...
2. **Match against team presets (before deriving anything)** — run `${SKILL_HOME}/scripts/match-team-preset.py --goal "<goal text>"` and act on the returned `confidence`: `high` → present the top preset (goal skeleton + roster + pattern) and **recommend reuse**; `medium` → present the top 2 candidates alongside the from-scratch option; `low`/`none` → say no preset matched and continue to step 3. A candidate flagged `patternKeywordsOnly` matched pattern vocabulary alone (zero of the preset's own signals) — it is **not** a preset match, MUST NOT be presented as a reuse candidate, and the engine already caps `confidence` at `none` when the top-ranked candidate carries that flag. Presets are known-good shapes distilled from teams that actually ran — reusing one is what keeps a vague goal from producing an arbitrary team. Never instantiate a preset silently, and never let it override an explicit user instruction. See `references/team-presets.md`.
