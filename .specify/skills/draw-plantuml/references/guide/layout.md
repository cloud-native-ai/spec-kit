...
| **回边 / 反馈边**（下层指回上层） | 拉长主流方向，占满整条高度 | 保留但用虚线 `..>` 弱化；或用 `-[hidden]-` 不参与主流排序（**两端已有可见边时 MUST NOT 这么叠**：构成 2-环，硬规则与锚点列替代手段见 [../sds-realization.md §2.1](../sds-realization.md)） |
...
- **同一对节点已有可见边时，MUST NOT 再叠加反向隐藏边**（构成 2-环，dot 反转不确定）——硬规则与不可见锚点列替代手段见 [../sds-realization.md §2.1](../sds-realization.md)
