...
**Definition-first is the core discipline.** A tool record exists to carry the user's authoritative knowledge about a tool, which **overrides the agent's built-in training knowledge** at invocation time. Therefore mandatory fields MUST come from the user or from a verified source — never auto-populated from what the model believes it knows about a command. Discovery assists only by bootstrapping a `Draft` for the user to complete.
