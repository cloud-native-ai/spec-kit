...
| Sub-agent | Stage | Template | Role-scoped responsibility |
|-----------|-------|----------|----------------------------|
...
| Evaluator | `evaluator` (Meta) | `agent-stage-evaluator-template.md` | Scores the deliverable on my role-default dimensions (see below), never sees the executor's prompt |
| Optimizer | `optimizer` (Meta) | `agent-stage-optimizer-template.md` | Adjusts the executor's environment + prompt to raise the next score |
