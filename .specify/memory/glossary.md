...
| 盲检 (Blind Check) | blind check, 盲检类, 空真断言 | 一条验证命令返回绿或空的结果,但成立理由与它要判的命题无关;本仓最危险的缺陷形态——红会被修,绿会被下游当证据引用 | auto | proposed |
| 变异演练 (Mutation Drill) | mutation drill, 变异测试演练 | 守卫一个否定命题时的有效性证据:破坏被守对象→确认转红→复原→确认转绿→删除探针并确认其消失;当前树上通过不算证据 | auto | proposed |
| 反空真哨兵 (Anti-Vacuity Sentinel) | anti-vacuity sentinel, 反空转哨兵 | 与每条『过滤后集合为空』断言配对的一条伴随断言,要求某个相关量非空,使『因正确而空』与『因失明而空』可区分 | auto | proposed |
| 同作者委派 (Same-Author Delegation) | same-author delegation, 同作者检测委派 | 被分析的制品由当前正在分析它们的代理在同一会话内写成时,检测必须委派给 fresh-context 只读子代理;真源 shared/workflow/objective-analysis-gate.md | auto | proposed |
