<!--
Sync Impact Report
- Version change: 1.9.1 → 1.10.0 (MINOR; Principle XI materially extended — two normative rules added from user-directed concept work under Dogfooding, 2026-08-14)
- Modified principles: XI. Dogfooding (Self-Application) — added "Fix the mechanism, not just the instance (修复落机制侧)" (repairs of mechanism-produced artifacts MUST land mechanism-side; instance patches only as logged temporary stabilization) and "Two hats: framework sources vs client runtime (两顶帽子)" (fixes intended for all consuming projects MUST land in framework sources and ride publish→install→init; direct `.specify/` runtime edits are client-side instance fixes)
...
- Templates requiring updates:
  ✅ shared/definitions/dogfooding-definitions.md — NEW concept authority (§1 problem/mechanism + fix classification; §2 client/framework + three-copy topology + two-hats rule; §3 glossary anchors)
  ✅ .specify/memory/glossary.md — four entries registered (问题修复/机制修复/框架项目/客户项目, origin=user, status=confirmed)
  ⬜ templates/instructions-template.md — the live-instructions propagation gap that motivated this amendment (## Dogfooding Practice section never reconciled into live .specify/instructions.md) is queued as a MECHANISM fix under the new rule: repair the /speckit.instructions reconcile flow + add a guarding contract (live managed-section set ⊇ template section set); not yet executed by user directive
- Follow-up TODOs: the mechanism fix above; test_c4_no_new_memory_layout remains a pre-existing baseline failure (stale pin over evidence/todo/tools dirs, unrelated to XI)
