...
**Concept authority (read-only)**: `shared/definitions/derivation-definitions.md` — the four record schemas, the provenance grades, chain rules C1–C7, the self-audit set A1–A14, the closed banned-justification literal set and the capability-degradation rule are defined there. This record describes engine *behavior* and re-defines none of the model.
...
| `.specify/specs/048-derive-command/contracts/derivation-model.md` | enforcement face of C1–C7 and A1–A14, `normalize_text`, the warning set, degradation |
...
- **RULE-1 — the anchor owns the model.** Schemas, the grade set, C1–C7, A1–A14, the banned-justification literals and the degradation rule come from `shared/definitions/derivation-definitions.md`. The engine holds *pinned copies* of the literals (`BANNED_JUSTIFICATIONS`) and of the small-file threshold (RULE-19); a drift test asserts each pinned copy equals its owner. Never treat this record, or the engine's constants, as the definition.
...
- **RULE-11 — the self-audit is a two-pass loop, because `validate` is zero-write.** A1–A10, A12 and A13 are **derived** from `errors[]` (each A row maps a closed set of error codes; zero hits → `pass`), and the artifact's own `## Self-Audit` rows must equal those derived values or the engine reports rule `A0`, `code: audit-result-diverges`. Consequences worth knowing before you call it: a freshly scaffolded archive carries `pending` in all fourteen rows and therefore does **not** validate clean — it exits 4 with twelve `audit-result-diverges` errors. The converging sequence is: run `validate` → transcribe `payload.audit.engine` into the A1–A10/A12/A13 rows → set A11 and A14 to `attested` with a checkable `method` sentence → run `validate` again. A11/A14 are agent attestations the engine never judges `pass`; their result is closed to `attested` / `not-attested` / `pending`, and `pending` is a to-do, not a failure. A `method` cell that is empty, `engine` or `n/a` draws the `attestation-method-degenerate` warning.
