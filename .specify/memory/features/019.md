...
Spec 015 (role-based-agents): Refactors the agent template approach from capability-based (common/knowledge/plan/research) to role-based (Requirements Analyst, System Designer, Module Designer, Test Engineer, QA Engineer, Knowledge Manager). Old `templates/agent-*-template.md` files are replaced by six role-based templates that generate project-aware agents with defined workflow handoffs. Plan phase completed with data model, handoff chain, context placeholder design, and two new companion skills (create-agent, improve-agent) for template evolution.
