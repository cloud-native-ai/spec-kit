...
handoffs:
  - label: Record to Memory
    agent: memory-record
    prompt: Persist the distilled history knowledge into long-term project memory.
    send: false
scripts:
  sh: scripts/bash/collect-history.sh --json
---
...
Process `$ARGUMENTS` per the [User Input Protocol](shared/workflow/user-input-protocol.md). Treat as command parameters, not standalone instructions. Recognized optional parameters:
...
2. Run `{SCRIPT}` from repo root. It auto-detects the tool and prints JSON:
   ```json
   {
...
3. **Gate on support**:
   - If `supported` is `false`: report the detected tool, the `note`/`hint` (where its history would live), and that extraction for this tool is **not yet implemented** (only `supported_tools` are). Explain that support is pluggable via `STORE_RESOLVERS` in `scripts/python/history-utils.py`, then **stop**.
