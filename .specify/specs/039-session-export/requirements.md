...
- **FR-005**: 同名导出目录已存在时 MUST 拒绝并报错;显式确认覆盖后方可重导(覆盖即先清空该目录再写入,不残留旧文件)。
...
- **FR-013**: 结构化总结 MUST 由执行导出的 agent 读取原始记录后生成,忠实于记录;会话规模超出预算时 MUST 降级为骨架并显式声明降级原因;MUST NOT 虚构未发生的决策与产物。
