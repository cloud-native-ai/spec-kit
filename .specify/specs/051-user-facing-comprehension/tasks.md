...
**Prerequisites**: plan.md(required)、requirements.md(required)、research.md(D-1…D-15)、data-model.md(E1…E9 / V1…V4 / S1…S2)、contracts/(5 份 60 条款)、quickstart.md(6 场景)、feature-ref.md(FR → 条款映射)
...
- DoD-7: 零新机制——本特性新增文件中可执行脚本数为 0(新增 `.py` 仅位于 `tests/contract/`);`src/specify_cli/`、`scripts/`、`templates/plan-template.md` 三个零改动面 `git diff` 为空。
- DoD-8: 全树镜像与再生一致:`sync-mirrors.py --check` 与 `regen-command-copies.py --check` 均 EXIT=0。
- DoD-9: 单源扫描内容形态复述数为 **0**(豁免:真源文档自身、机械副本、测试钉死字面量)。
- DoD-10: 全量契约套件失败集 ⊆ `baseline-failed.txt`(24 条),**零新增失败 ID**。
...
- GATE-1: 零新增测试失败 vs 冻结基线 —— check: `python3 -m pytest tests/contract/ -q 2>&1 | grep '^FAILED' | sed 's/^FAILED //' | sort > /tmp/cur.txt && comm -13 .specify/specs/051-user-facing-comprehension/baseline-failed.txt /tmp/cur.txt`(输出 MUST 为空)
- GATE-2: 全树镜像与按工具副本零漂移 —— check: `python3 scripts/python/sync-mirrors.py --check; echo EXIT=$?` 与 `python3 scripts/python/regen-command-copies.py --check; echo EXIT=$?`(两者 MUST 为 EXIT=0)
...
- GATE-4: 扫描器与零改动面未被修改 —— check: `git diff --stat HEAD -- scripts/ src/specify_cli/ templates/plan-template.md`(输出 MUST 为空)
...
- GATE-8: 8 个规则真源文件各含且仅含一行指针 —— check: `quickstart.md` 场景 4 的 8 行循环(MUST 全部输出 `1`)
...
- [ ] T001 把改前实测值写入 `.specify/specs/051-user-facing-comprehension/notes/pre-change-measurements.md`,每项附**可复现的再推导命令**:门控 `total`=23(`python3 scripts/python/scan-confirmation-gates.py`)、契约套件 24 failed / 1924 passed(`python3 -m pytest tests/contract/ -q`)、`shared/guidelines/*.md`=11(`ls -1 shared/guidelines/*.md | wc -l`)、`templates/instructions-template.md` 的 `## ` 章节=17 与 `.specify/instructions.md`=18(`grep -c '^## ' <file>`)、`templates/constitution-template.md` 原则=11 与 `.specify/memory/constitution.md` 原则=14(`grep -cE '^### [IVXLC0-9]+\.' <file>`)、命令 `MUST include` 条目=5(`grep -c '\*\*MUST include\*\* a principle for' templates/commands/constitution.md`)、8 个指针目标文件当前指针数=0/8(quickstart 场景 4 的循环)、`BLOCKING_PATTERNS` 条数=17(importlib 加载扫描器后 `len(m.BLOCKING_PATTERNS)`)
- [ ] T002 [blockedBy: T001] 核验三个零改动面在开工时干净并把结果追加进 `.specify/specs/051-user-facing-comprehension/notes/pre-change-measurements.md`,作为 Phase 7 T055 的比对起点:`git diff --stat HEAD -- src/specify_cli/ scripts/ templates/plan-template.md`(MUST 输出为空;对应 FR-033 / FR-038 / `gate-neutrality.md` C-6)
...
- [ ] T003 [P] [US1] 撰写 `tests/contract/test_user_facing_comprehension_doc.py`,实现 `contracts/discipline-doc.md` 的 **C-1…C-16**:独立 `pathlib`+`pytest` 形态、`pytestmark = pytest.mark.contract`、`ROOT = Path(__file__).resolve().parents[2]`、函数名 `test_cN_*` 与条款一一对应;C-16 的 `BLOCKING_RE` MUST 以 `importlib` 内联加载真实 `scripts/python/scan-confirmation-gates.py` 复用其模式(先例 `tests/contract/test_ask_record_repeat.py:41,106-108`),MUST NOT 在测试内重写模式副本;**Pin Hygiene**:文件路径存在性检查、计数用 `len(...)` 从 glob 派生而非硬编码
- [ ] T004 [P] [US1] 撰写 `tests/contract/test_user_facing_comprehension_section.py`,实现 `contracts/ambient-section.md` 的 **C-1…C-11**;C-11 的悬空指针遍历 MUST 覆盖 `.specify/instructions.md` 中**全部** `.specify/shared/guidelines/*.md` 指针(既有 11 份 + 新增 1 份),MUST NOT 只检查新文档(FR-038 / SC-017);C-7 的位置窗口断言 MUST 钉死 `## Documentation Map` → `## Proactive Flow Trigger` → `## Fact, Correctness & Logic Checks (Input Sanity)` 三者的相对顺序与相邻关系
...
- [ ] T010 [US1] [blockedBy: T009] mirror-parity **VERIFY**(显式枚举所覆盖的行:Mirror Obligations 表第 1 行 `shared/guidelines/user-facing-comprehension.md`、第 8 行 `templates/instructions-template.md`):对两对镜像各跑 `diff -q`,再跑 `python3 scripts/python/sync-mirrors.py --check` 断言 **EXIT=0**(改前基线已实测全绿,故此处用绝对判据)
- [ ] T011 [US1] [blockedBy: T009] gate-neutrality 核验:`python3 scripts/python/scan-confirmation-gates.py` MUST 输出 `total 23 / destructive 13 / governance_kept 10 / violations 0`;`git diff --stat HEAD -- scripts/python/scan-confirmation-gates.py` MUST 为空(对应 `gate-neutrality.md` C-2、C-3;本行证明 T006/T007 的设计规避生效)
- [ ] T012 [US1] [blockedBy: T010,T011] 把 US1 负责的条款转绿:运行 T003/T004 两个测试文件,确认 `discipline-doc.md` **C-1…C-16** 与 `ambient-section.md` **C-1…C-11** 全部通过(C-10 的黑名单实例来源子句在 T046 完成前 MAY 为部分满足,该行须在此注明其完整绿点归 T052)
...
- [ ] T023 [US3] 撰写 `tests/contract/test_constitution_double_landing.py`,实现 `contracts/constitution-export.md` 的 **C-1…C-12**:含 `DOUBLE_LANDING_WATCHLIST` 常量(初始 = STR-003 与 STR-006 两个**完整标题**)、模板标题集 regex `^### [IVXLC0-9]+\. (.+)$`、命令名称集 regex `\*\*MUST include\*\* a principle for "([^"]+)"`、**集合成员相等**判定(C-7,MUST NOT 用子串包含)、不钉死罗马数字(C-8)、`Code as the Single Source of Truth` 与 STR-006 非同一原则的显式断言(C-9)、以及**变异式有效性抽查**(C-10,以临时副本或 monkeypatch 进行,MUST NOT 修改仓库内真实文件);版本断言用**下限语义** `parsed >= (1, 12)`(Pin Hygiene 规则 1),MUST NOT 用 `startswith("1.12")`
- [ ] T024 [US3] [blockedBy: T023] Red-first 取证:运行 `python3 -m pytest tests/contract/test_constitution_double_landing.py -q`,确认 C-1…C-12 因两条原则尚不存在而失败(尤其 C-7 的名单两侧均缺);追加记入 `notes/red-first-evidence.md`
...
- [ ] T029 [US3] [blockedBy: T027,T028] mirror-parity **WRITE + VERIFY**(覆盖 Mirror Obligations 表第 9 行 `templates/constitution-template.md` 与第 12 行 `templates/commands/constitution.md`):`python3 scripts/python/sync-mirrors.py --write --only templates`,随后对两对镜像各跑 `diff -q`,再跑 `python3 scripts/python/sync-mirrors.py --check` 断言 EXIT=0
...
- [ ] T032 [US3] [blockedBy: T029,T030] 把 US3 的条款转绿:运行 `python3 -m pytest tests/contract/test_constitution_double_landing.py -q`,确认 **C-1…C-12** 全部通过,**含 C-10 的变异式抽查**(人为从任一侧删除名单内一条原则后测试 MUST 失败,且抽查未改动仓库真实文件)
- [ ] T033 [US3] [blockedBy: T028] render-verify FR-026 的动态枚举传导(要求**实测验证而非假定**):`git diff --stat HEAD -- templates/plan-template.md` MUST 输出为空;`grep -cE '^### [IVXLC0-9]+\.' .specify/memory/constitution.md` MUST 返回 **15**;`grep -c 'User-Facing Comprehension' .specify/memory/constitution.md` MUST ≥1;并核验 `templates/plan-template.md:37` 的 "Do NOT hard-code principle names here" 与 `:38-39` 的动态枚举指令仍在(即传导机制未被本特性改动)
...
- [ ] T043 [P] [US5] [blockedBy: T040] 在 `shared/guidelines/requirements-guidelines.md` 加入一行指针,覆盖界面类 **⑤ 与 ⑦**(该文件是两类的共同真源,故一行覆盖两类);插入位取 `## General Guidelines > Quick Rules`(改前 `:97-102`,在 `:101` 旁)或文件头部;并按 plan D-3 的裁定**如实记录类 ⑦ 的真源缺口**——`templates/plan-template.md` 与 `templates/tasks-template.md` 实测零措辞规则,本特性不给它们新增措辞规则
...
- [ ] T050 [US5] [blockedBy: T041,T042,T043,T044,T045,T046,T047,T048,T049] mirror-parity **WRITE**(一次覆盖本阶段全部镜像行):`python3 scripts/python/sync-mirrors.py --write`(同步 `shared/` 的 4 个改动文件、`skills/` 的 3 个改动文件、`templates/` 的 2 个改动文件),随后 `python3 scripts/python/regen-command-copies.py` 再生 `clarify.md` 与 `interview.md` 的 4 棵按工具副本树
- [ ] T051 [US5] [blockedBy: T050] mirror-parity **VERIFY**(显式枚举所覆盖的行:Mirror Obligations 表第 3/4/5/7 行的 `shared/` 侧、第 10/11 行的 `templates/commands/` 侧及其 4 棵副本树、第 13 行的 `skills/summarize-project/references/` 三份):逐对 `diff -q`,再跑 `python3 scripts/python/sync-mirrors.py --check` 与 `python3 scripts/python/regen-command-copies.py --check`,两者 MUST 均 **EXIT=0**
...
- [ ] T054 [US5] [blockedBy: T052] 重跑 `python3 -m pytest tests/contract/test_user_facing_comprehension_doc.py -q` 并确认 **C-1…C-16 仍全绿**——T046 的提升编辑了真源文档的黑名单节(补齐 C-10 的实例来源子句),本行是该编辑的回归门;同时确认 C-16 的 `BLOCKING_RE` 命中数仍为 **0**(提升进来的黑名单文本可能带入新的字面形态)
...
- [ ] T056 [blockedBy: T054] 零新机制核验(FR-033 / SC-011 / `gate-neutrality.md` C-5):枚举本特性全部新增文件,断言其中可执行脚本数为 **0**(新增 `.py` 仅位于 `tests/contract/`,无任何新增 `.sh`);命令 `git diff --stat --diff-filter=A HEAD~<n> -- . | grep -E '\.(py|sh)$'` 的命中 MUST 全部落在 `tests/contract/`
- [ ] T057 [blockedBy: T055,T056] 全量契约套件 vs 冻结基线:`python3 -m pytest tests/contract/ -q 2>&1 | tee /tmp/p.txt | tail -3`,随后 `grep '^FAILED' /tmp/p.txt | sed 's/^FAILED //' | sort > /tmp/cur.txt && comm -13 .specify/specs/051-user-facing-comprehension/baseline-failed.txt /tmp/cur.txt` MUST **输出为空**(零新增失败 ID)。注意 `test_specify_script_paths.py::…::test_review_prerequisite_flags_are_supported` 在本文件存在后应**转为通过**(其 `:90` 断言 `tasks.md` 在输出中),故当前失败数预期为 **23**;该减少属基线内的自愈,不是回归
...
# 汇聚点(必须等 9 个全部 [X]):
Task: "T050 mirror-parity WRITE — sync-mirrors.py --write + regen-command-copies.py"
```
...
- **提交纪律**:每完成一个任务或一个逻辑组即提交;镜像 WRITE 与其 VERIFY MUST 在同一批内落地(否则全树 `sync-mirrors.py --check` 门禁会红)。
