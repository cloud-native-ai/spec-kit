...
- **全树镜像字节相等**:`test_scripts_distribution_parity.py:71-87` 对整棵树跑 `sync-mirrors.py --check` 并断言 EXIT=0 ⇒ 每处源改动 MUST 同批再生镜像。**但本特性 MUST NOT 以"全树 EXIT=0"作判据**(订正发现项 B-03):计划期"改前基线已实测全绿"的前提已于 2026-09-18 被**本特性未触及**的 `skills/draw-diagram/` 在途改动推翻(全树现为 EXIT=2,而本特性触及范围仍为 EXIT=0;归因证据与探测命令见 `tasks.md` § Environment Prerequisites——该节是探测结论的唯一落点)。绝对判据在此只有两个坏结果:要么本特性被无关状态无限期阻塞,要么执行者跑裸 `sync-mirrors.py --write` 把别人的在途工作吸收进本特性的提交以"转绿"(即发现项 V-01)。故判据取 **049 的先例形态**——"所触及镜像对报 `ok`,且相对 T001 冻结的既有漂移集**无新增 `MISS`/`DIFF` 行**",而非命令整体 exit 0;`regen-command-copies.py --check` 今天仍干净,故它保留绝对判据。另需注意 `test_scripts_distribution_parity.py` 本身就断言全树 EXIT=0,因而今天失败,MUST 由 T001 纳入重冻的 `baseline-failed.txt`,不得报成本特性回归。
...
改前基线已实测(2026-09-17,**现为已过期快照,仅作日期化记录、MUST NOT 当作当前真源引用**):`python3 scripts/python/sync-mirrors.py --check` → **EXIT=0**(`ok templates/ == .specify/templates/ (22 files)`、`ok skills/ == .specify/skills/ (489 files)`、`ok agents/ == .specify/agents/templates/ (2 files)`、`ok scripts/ == .specify/scripts/ (104 files)`、`ok shared/ == .specify/shared/ (40 files)`,外加 18 条非致命 `note extra file only in mirror:`);`python3 scripts/python/regen-command-copies.py --check` → **EXIT=0**(该命令今天仍干净)。

**订正(发现项 B-03,2026-09-18 实测)**:上述"全树 EXIT=0"的前提已被**本特性未触及**的在途改动推翻——`python3 scripts/python/sync-mirrors.py --check` 现为 **EXIT=2**,`DIFF .specify/skills/draw-diagram/SKILL.md` 与 `MISS .specify/skills/draw-diagram/references/self-deploy-render-service.md`(归因:二者是未提交的在途改动,mtime 2026-09-18 10:48/10:49 晚于本基线,且对 `user-facing-comprehension` 命中数为 0);而本特性触及范围 `--check --only shared --only templates --only skills/summarize-project` 实测仍为 **EXIT=0**。故本表的判据由"绝对判据(全树 `--check` EXIT=0)"改为 **049 的先例形态**:"所触及镜像对报 `ok`,且相对 T001 冻结的既有漂移集**无新增 `MISS`/`DIFF` 行**";`regen-command-copies.py --check` 保留绝对判据。**归因订正**:原文写"无需 **050** 那样的 `--only` workaround",归属错误——`--only` 范围化是 **049** 的 workaround(`049/plan.md` 明写其判据是"所触及镜像对报 `ok` 且无新增漂移行,而非命令整体 exit 0";`049/tasks.md` 明令 "Never run broad `sync-mirrors.py --write`"),而 **050** 恰恰是**退役**该 workaround 的特性(其 requirements 记录"Feature 049 记录的 git-fleet 6 MISS 基线已失效…049 的 `--only` 范围化 workaround 不再需要")。既有漂移的探测命令与归因证据落在 `tasks.md` § Environment Prerequisites,重冻由 T001 承担;`research.md` D-15 只覆盖 pytest 基线、**不**覆盖镜像基线,故镜像侧的重冻义务由本段与 T001 共同承载。
