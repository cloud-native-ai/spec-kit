...
- **特殊名滥用**：普通文档使用大写保留名（或特殊文件被小写化）属确定性违规，点名并给出重命名建议；重命名属移动类动作，须经干跑计划确认。
...
- **FR-003**: 命令 MUST 按调谐模式实现为单一引擎：作用域判定（无参全量 / 单目标定向 / 原始材料扇出 / 空间为空 bootstrap）+ 调谐环（观察快照 → 期望态计算 → 容忍带优先的分维度差异 → 干跑计划 + 分级确认 → 收敛 → 校验与残差报告）；MUST 产出四件强制产物（观察快照、干跑计划、审计日志、残差报告，审计日志即使零收敛也要落盘），其中需落盘的运行产物（干跑计划、审计日志等）MUST 落在 `.specify/` 下的命令工作区，不混入读者可见的 `docs/`；期望态来源优先级 MUST 为：模板 < 规则阈值 < 原则 < 外部权威事实 < 本地既有惯例 < 本次用户输入。MUST NOT 以新增顶层模式的方式扩展能力。
...
- **FR-010**: 期望态基线 MUST 包含命名语义规则：特定名称的文档具有固定含义，**文件名本身是语义的一部分**；此类特殊文档的文件名 MUST 全大写。当前已注册的特殊文件及其固定语义为：根目录薄层 `README.md`（索引 `docs/` 全部）、`ARCHITECTURE.md`（摘要 `docs/concepts/` + `docs/decisions/`）、`CONTRIBUTING.md`（摘要 `docs/contribute/`）、`CHANGELOG.md`（自包含时间线）。特殊名注册表 MUST 可扩展（新增特殊名须同时登记其固定语义）；普通文档 MUST 使用小写 kebab-case 且 MUST NOT 复用保留大写名（与 constitution 既有文档命名原则一致）；大写保留名合规属于确定性差异维度（FR-007）。
...
- **特殊文档注册表（Special-Name Registry）**: "文件名即语义"的大写保留名清单及各自固定含义（当前：README/ARCHITECTURE/CONTRIBUTING/CHANGELOG），可扩展、新增须登记语义；作为期望态基线与确定性校验的一部分。
