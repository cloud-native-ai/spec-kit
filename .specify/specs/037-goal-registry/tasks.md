...
- [X] T061 [P] Propose glossary entries for the new terms (Team Territory, Team Roster, Overlap Finding, Contested Area, Coordination Round) with `origin=auto`, `status=proposed`, run conflict detection, and obtain user confirmation before writing to `.specify/memory/glossary.md` <!-- verified: 5 glossary terms added (Team Territory/Team Roster/Overlap Finding/Contested Area/Coordination Round), origin=auto status=proposed; no conflict with existing rows -->
- [X] T062 [P] Author a Tool record for `goal-utils.py` at `.specify/memory/tools/goal-utils.py.md` — Draft status until verified by execution [blockedBy: T022] <!-- verified: .specify/memory/tools/goal-utils.py.md authored (Draft): 6 subcommands, 9 behavioral rules, exit codes, Draft until CI-verified on the >=3.8 floor -->
- [X] T063 [P] Update `.specify/memory/features/041.md` and the `features.md` row with the implemented state and any deferred tasks <!-- verified: features/041.md + features.md row flipped Planned->Implemented with the 17-pass/1-partial tally and deferred SC-016 note -->
- [X] T064 Live-face residual sweep: classify `grep -rl 'project/goal'` by face using the rule in `requirements.md` § "SC-011 Source"; the live subset MUST be empty and the 18 historical files (`.specify/specs/036-team-summary/**`, `.specify/specs/037-goal-registry/**`, `.specify/memory/feedback/**`, `.specify/memory/features*`) MUST be byte-unchanged [blockedBy: T012,T026,T055] <!-- verified: live-face residual sweep: 0 live occurrences of project/goal; 25 historical preserved; 4 guards name the old path by design -->
- [X] T065 Full mirror verification: `python3 scripts/python/sync-mirrors.py --check` exits 0, covering every row of plan.md's Mirror Obligations table [blockedBy: T012,T026,T034,T055,T060] <!-- verified: sync-mirrors --check exit 0 across every Mirror Obligations row (skills, templates+5 copies, scripts incl. goal-utils.py, Tool record source) -->
- [X] T066 Definition read-only proof: fingerprint a goal's `goal.md`, run a refresh, re-fingerprint — identical [blockedBy: T050] <!-- verified: goal.md sha256 byte-identical across a refresh — SC-008 read-only holds -->
- [X] T067 [P] **Measure SC-010**: scan `skills/create-team/references/goal.md`, `skills/improve-team/references/goal-editing.md` and `skills/create-team/references/optimization-goals.md` (plus their mirrors) for residual statements that a goal belongs inside team files; the count of second-account statements MUST be 0 and every concept statement MUST link to `shared/definitions/goal-definitions.md` [blockedBy: T031,T032,T033] <!-- verified: SC-010 measured: 3/3 guides link to goal-definitions.md (goal-editing.md fixed this phase), 0 second-account statements -->
- [X] T068 [P] **Measure SC-012**: count the surfaces able to write a goal definition (commands, skills, scripts that write under `.specify/goal/**`) — MUST be exactly 1 (`/speckit.goal` via `goal-utils.py`); record the count, not just the existence of the command [blockedBy: T026,T065] <!-- verified: SC-012 measured: exactly 1 definition writer (goal-utils.py); sync-mirrors --check exit 0 -->
- [X] T069 [P] **Measure SC-013**: run the eight-dimension conformance check of `requirements.md` against `shared/definitions/goal-definitions.md` (composition, lifecycle set, plane relation incl. no structural link, criteria authority, singularity, object scope, narrative shape, verification mode); conflicts MUST be 0 [blockedBy: T002] <!-- verified: SC-013 measured: 8-dimension conformance vs goal-definitions.md, 0 conflicts -->
- [X] T070 [P] **Measure SC-014**: cross-compare every archived `goal.md` against every `requirements.md` in the repo — verbatim criteria duplication MUST be 0, `requirements.md` files carrying a goal field MUST be 0, and goal definitions enumerating FRs MUST be 0 [blockedBy: T028] <!-- verified: SC-014 measured: 0 criteria duplication, 0 requirements.md with a goal field, 0 goal defs enumerating FRs -->
- [X] T071 Full suite and baseline diff: `pytest -q`, zero new failure names vs `baseline-failed.txt`. Note that `test_review_prerequisite_flags_are_supported` should now pass again because `tasks.md` exists [blockedBy: T064,T065] <!-- verified: final full gate: 39 failed / 1480 passed / 1 skipped; comm vs baseline = 0 new; review-prereq now passes (tasks.md exists) -->
- [X] T072 Write `verification.md` with a status line for every SC-001…SC-018, the measured value for each, and `deferred_tasks=` if any row is `[~]` — every status MUST cite the task that produced its measurement, never an assertion [blockedBy: T067,T068,T069,T070,T071] <!-- verified: verification.md written: all 18 SC statuses each citing a measuring task; 17 pass / 1 partial (SC-016 live-ratification half) / 0 fail -->
