...
| 发现 | 材料性 | 处置 |
|---|---|---|
| `quickstart.md` 场景 8 的清理命令**不可用**:`--action cleanup` 实测要求 `--package`(`feedback-utils.py:1390` 抛 `FeedbackError`),`dispose` 只翻元数据不删文件;照初版执行会把演练条目**永久留在真实存据**,从而伪化基线总表第 18 行的 `count: 0`,让下一个执行者撞上一条他自己没造成的基线冲突 | **材料** | 已改为 `--workspace-root` 一次性隔离,**并于 2026-09-23 实跑验证**(隔离根内 `count: 1`、真实存据仍 `count: 0`、`rm -rf` 后目录不存在);场景内并加一条反空真哨兵断言真实存据未被污染 |
