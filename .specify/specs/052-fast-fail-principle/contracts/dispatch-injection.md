...
> *本条的 **6** 是 052 落地时点的行数;行数的现行拥有者是守卫 `test_i13`(2026-09-24 复核:该表因本特性之外的改动增入 `incremental_landing`,今为 7 行,守卫同批要求两行同时在场)。本条的规范内容是「`fast_fail_clause` 行在场且指向拥有者字面量」,不是行数本身。*
