...
> Archived Note: 本规范描述的是历史 MCP-only 需求，当前统一命令入口为 `/speckit.tools`（由 `004-speckit-tools-command` 延续）。

**Requirement Branch**: `002-mcp-tool-call`  
**Created**: 2026-02-10  
**Status**: Draft  
**Input**: User description: "新建一个 MCP 工具调用命令，用来指定调用某一个 MCP 工具。工具主要信息包括：所属 MCP server、tool 描述、参数和返回值等。命令调用后需要通过自动发现和交互补全这些信息，然后在 .specify/memory/tools/<mcp tool name>.md 记录；每次调用时先判断记录是否存在，存在则直接复用，否则按流程生成。"

## User Scenarios & Testing *(mandatory)*

### User Story 1 - 选择并调用 MCP 工具 (Priority: P1)

作为使用 Spec Kit 的用户，我希望通过 `/speckit.tools` 指定并调用某一个 MCP 工具，以便快速完成一次工具调用而无需手动整理工具定义信息。
...
1. **Given** 用户在工作区内执行 `/speckit.tools` 并指定目标 MCP 工具，**When** 系统完成工具信息收集，**Then** 工具被成功调用并返回结果。
...
1. **Given** `.specify/memory/tools/<mcp tool name>.md` 已存在且内容完整，**When** 用户再次调用 `/speckit.tools`，**Then** 系统直接使用文件信息完成工具调用。
...
- **FR-001**: 系统 MUST 允许用户在调用 `/speckit.tools` 时明确指定目标 MCP 工具。
...
- **工具调用会话**: 一次 `/speckit.tools` 调用过程中的上下文、交互记录与执行结果。
...
- **SC-001**: 95% 的 `/speckit.tools` 调用可在一次流程中完成工具发现/复用并成功执行。
