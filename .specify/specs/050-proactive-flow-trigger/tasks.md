...
- DoD-8: Tool 记录 `.specify/memory/tools/trigger-utils.py.md` 已建,且 `.specify/memory/tools.md` 与 `.specify/tools/*.json` 经 `refresh-tools.sh` **再生**(未手改派生清单) <!-- CORRECTED 2026-09-08 (implement): `.specify/memory/tools.md` is NOT regenerable — see §Implementation-Period Corrections C-1. Satisfied for the record + `.specify/tools/*.json`; the tools.md clause is void, not skipped. -->

**DoD Status**: green   <!-- flipped 2026-09-08 by /speckit.implement after re-validating every
     row against the current tree rather than trusting the all-tasks-done state.
     DoD-1 50 [X] + 1 [~] (T050, reason recorded), 0 open tasks.
     DoD-2 8 files / 313 tests green, collection asserted per file, name-level regression empty.
     DoD-3 61/61 clauses have a named assertion (12 + 23 + 10 + 16).
     DoD-4 7 rows: 5 mirror pairs byte-identical, row 6 verified by regen-command-copies --check
           exit 0, row 7 record count 9 -> 10.
     DoD-5 total 23 == frozen 23, violations 0; 19 / 4 / 16 and probe 73 unchanged.
     DoD-6 all 6 quickstart scenarios walked, evidence in verification.md.
     DoD-7 13 pass + 2 deferred-with-reason, no other statuses.
     WARNING DoD-8 is satisfied EXCEPT one VOID sub-clause: `.specify/memory/tools.md` cannot be
     "regenerated by refresh-tools.sh" because nothing in the repo writes that file and the script
     itself exits 1 without a source flag. The Tool record and `.specify/tools/*.json` halves ARE
     done. Measured four ways and documented in Implementation-Period Corrections C-1 below;
     recorded as void, NOT silently treated as met. -->
...
- [X] T037 [P] [US4] 编写集成测试(RED)`tests/integration/test_trigger_tuning.py`:`tune` 的每条提议含证据字段(指标 + 样本数);`stats.hits < minSample` 的规则**不出现**在提议中且被 `notes[]` 具名回显(FR-017 小样本守卫);`tune` 后未 `tune-apply` → 规则集**零变更**(FR-016);`tune-apply --proposal p-001 --reason ...` 后 SM-2 走 `ratified → applied` 且写 `tuning.{ratifiedAt,evidenceRef}`;`record-manual --flow X` 在当回合遥测 `suggested=false` 时产出漏报证据并被下一次 `tune` 消费;需 agent 判断的项出现在 `semanticJudgmentPending[]`

### Implementation for User Story 4

- [X] T038 [US4] 在 `scripts/python/trigger-utils.py` 实现 `record-manual` action:`--flow` 必填;与当回合遥测的 `suggested` 标志关联,`suggested=false` 时记为**漏报证据**(FR-015;首轮 Q1=A 唯一通道的补偿控制) [blockedBy: T031,T037]
- [X] T039 [US4] 在 `scripts/python/trigger-utils.py` 实现 `tune` action:确定性聚合命中率 / 拒绝率 / 误报 / 漏报差集(手动调用集 − 已建议集);**小样本守卫** `stats.hits < config.minSample`(默认 5)的规则 MUST NOT 产出提议,被跳过者在 `notes[]` 具名;每条提议含 `proposalId`(`p-nnn`)/ 类型(收紧·抑制·新增)/ 证据(指标 + 样本数);语义判断(该不该采纳、新规则配哪条流程)经 `semanticJudgmentPending[]` 交回 agent(承 `derive-utils.py` 的"确定性交引擎、语义留 agent"分工) [blockedBy: T038]
- [X] T040 [US4] 在 `scripts/python/trigger-utils.py` 实现 `tune-apply` action:`--proposal <p-nnn>` + `--reason` 必填;SM-2 `ratified → applied`;写 E2.`tuning.{suppressedBy,ratifiedAt,evidenceRef}`;未经该 action 规则集 MUST NOT 变更(FR-016) [blockedBy: T039]
- [X] T041 [US4] mirror-parity 写 + 验(Mirror Obligations 第 3 行,STRICT):`python3 scripts/python/sync-mirrors.py --write` + `diff -q` + `--check` exit 0 [blockedBy: T040]
- [X] T042 [US4] 验证:`python3 -m pytest tests/integration/test_trigger_tuning.py tests/contract/test_trigger_engine.py -q` 全绿 [blockedBy: T041]

### Manual Verification for User Story 4

- [X] T043 [US4] 人工走查 `quickstart.md` 场景 6:6a 漂移检出受控实验(临时改写某 `## Handoffs` 的流程名 → `test_trigger_seed_derivation.py` C-7 失败并指名 ruleId/file/flow → **恢复**,修复方式是同步种子而非放宽测试)+ 6b–6e 漏报证据与调优批准链;证据记入 `verification.md` 的 SC-007 / SC-013 条目 [blockedBy: T042]
...
- [X] T044 [P] 新建 Tool 记录 `.specify/memory/tools/trigger-utils.py.md`(Principle XII 义务;形态照既有 9 条记录:Tool Name / Type `project-script` / Source Identifier `scripts/python/trigger-utils.py` / Tool ID / Aliases / Status / Scope / Description / Invocation & I/O Contract / Parameters 表含 11 个 action);随后运行 `bash scripts/bash/refresh-tools.sh --json` **再生** `.specify/memory/tools.md` 与 `.specify/tools/{system,shell,project}.json`(派生清单禁手改),并断言清单中出现新 Tool [blockedBy: T042] <!-- CORRECTED 2026-09-08 (implement): the stated `refresh-tools.sh --json` invocation exits 1 (it requires a source flag) and nothing in the repo writes tools.md — see §Implementation-Period Corrections C-1. -->
- [X] T045 [P] 漂移守卫**活性**证明(防守卫空洞):在临时副本上把某个 `templates/commands/<x>.md` 的 `## Handoffs` 流程名改写,运行 `python3 -m pytest tests/contract/test_trigger_seed_derivation.py -q` 断言**失败**且错误信息同时含 `ruleId`、`provenance.file` 与缺失的 `flow` 值,随后**恢复原文件**并复跑断言绿。记录该受控实验证据(证明 C-7 不是恒真断言)[blockedBy: T016,T019]
- [X] T046 全套件名字级回归(GATE-1):`bash scripts/bash/run-tests.sh --names-out /tmp/cur-failed.txt` 后 `comm -13 .specify/specs/050-proactive-flow-trigger/baseline-failed.txt /tmp/cur-failed.txt` **输出为空**;同时记录新增通过数 [blockedBy: T027,T035,T042]
- [X] T047 全树门禁扫(GATE-2/3/4/7):`python3 scripts/python/sync-mirrors.py --check` exit 0;`python3 scripts/python/regen-command-copies.py --check` exit 0(**必须未变** —— 本特性不改任何命令模板);`python3 scripts/python/scan-confirmation-gates.py --summary` total **等于 T002 冻结前值**(撰写时实测 23)/ violations **0**;`python3 -m pytest tests/contract/test_confirmation_gates_sweep.py -q` 绿;`python3 scripts/python/feedback-utils.py --action probes --validate` exit 0 且 internal objects 仍 **73** [blockedBy: T046]
- [X] T048 零扰动计数复核(GATE-8):`python3 -m pytest tests/contract/test_feedback_command_classification.py tests/contract/test_docs_step_injection.py -q` 绿,且两文件内的 `COMPLEX_COMMANDS`(19)/`SIMPLE_COMMANDS`(4)/docs-step(16)**未被本特性改写**(P-Q1=A 的承诺:不新增命令即不动计数)[blockedBy: T046]
- [X] T049 [P] 撰写 `.specify/specs/050-proactive-flow-trigger/verification.md`:为 **SC-001…SC-015** 逐条写 `SC-NNN_status=pass|deferred` 行,附 requirements.md § Measurement Sources 指定的证据来源与本次实测读数;`deferred` 项须附因。**本任务汇总他人产出的读数,故必须后于全部产出任务**(analyze M-14:否则 GATE-6 / DoD-7 可能在证据尚未产生时就变绿)[blockedBy: T015,T028,T036,T043,T047,T050]
- [~] T050 Dogfood 实测(SC-011 / SC-012 的真实读数):在本仓以机制上线后的状态跑 **≥20 个真实回合**,用 `trigger-utils.py --action status` 读出实测 `escalationPct` 并与 `probeBudgetPct=20` 对比、读出无适用流程回合的 `visibleOutputCount`(期望 0)、核对 `complianceDone` 与 `suggested` 的时序(期望零 `ordering-violation`);**本任务另是 SC-008② 与 SC-004② 的具名产出者**:⑦ 记录 dogfood 期间「因建议导致用户当前请求被阻塞或延后」的**事件计数**(期望 0,SC-008②——此前无任何任务产出该读数);⑧ 记录同会话内同一情境重复评估时的**重复建议计数**(期望 0,验证 `trigger-engine.md` C-20 的会话级抑制,SC-004②);⑨ 诚实记录 SC-011 的分母口径局限(`data-model.md` V4.5:遥测无法证明 agent 跳过了某回合的评估,故须附会话侧观察)。全部记入 `verification.md` [blockedBy: T028,T036,T043] <!-- deferred: 2026-09-08 — requires >=20 turns of ORDINARY work with the mechanism live; this session was BUILDING the mechanism, so its turns are not genuine samples (the plan already listed T050 as a foreseeable [~] candidate). Achieved instead: the store was initialised in this repo and 3 real turns recorded, which produced live evidence for SC-004(2) session suppression (turn 1 suggested, turns 2-3 suppressed), 0 ordering-violations, 0 silence-violations and 0.0% escalation, plus confirmation of the V6.9 tracking boundary. Recorded in verification.md; SC-008 part 2 and SC-011 deferred with it. -->
- [X] T051 实现收尾的 Feature 记录义务(**不推进状态** —— `Planned → Implemented` 由 `/speckit.implement` 持有,受 Principle VII Pre-Status-Flip Gate 约束:零 `[ ]` 任务 + 每条 SC 在 verification 有状态行)。在 `.specify/memory/features/050.md` § Latest Review 追加 implement 阶段实测要点,并在 `.specify/memory/features.md` 050 行 Last Updated 追加 implement 注记(**手工编辑**,禁跑 `update-feature-index.sh`);若 T011 的路径覆盖修复已落地,确认 `features/022.md` 的对应注记仍在(该注记由 tasks 阶段写入)[blockedBy: T049,T050]
...
## Implementation-Period Corrections

Append-only. Recorded by `/speckit.implement` on 2026-09-08 when measurement contradicted a
task row. Rows above keep their original wording and IDs; these notes are the authority on
what was actually true.

### C-1 — `.specify/memory/tools.md` is not a regenerated inventory (voids part of T044 / DoD-8)

**Claim as written**: T044 and DoD-8 state that `bash scripts/bash/refresh-tools.sh --json`
regenerates `.specify/memory/tools.md` together with `.specify/tools/{system,shell,project}.json`,
and that the new Tool must then appear in that inventory.

**Measured reality** (four independent checks):

1. `bash scripts/bash/refresh-tools.sh --json` **exits 1** with a usage message — it requires a
   source flag (`--system` / `--shell` / `--project`) and prints JSON to stdout; it writes no file
   itself. The canonical caller is `generate-instructions.sh:39-41`, which redirects each source
   into `.specify/tools/{system,shell,project}.json`.
2. Nothing in `scripts/` or `src/` writes `tools.md` — neither by literal path nor by a
   fragment-built path (checked for `f"tools`, `"tools" +`, `TOOLS_MD`, and every `>`/`tee`
   redirection in `refresh-tools.sh`).
3. `.specify/memory/tools.md` is a 1984-line **MCP server index** (`# Helper Tools Index`,
   `## MCP Tools`, timestamp 2026-02-02) whose `## System Binaries` / `## Shell Environment` /
   `## Project Scripts` sections contain **only a one-line description each, never an enumeration**.
4. It was **born that way**: the revision that added it (`15b13dba`) already had exactly this
   structure. Its only later commits are hand-edits (`update documents`). So the claim was never
   true, not merely stale.

**Resolution**: T044 is satisfied for what is real — the definition record
`.specify/memory/tools/trigger-utils.py.md` was authored in house shape (record count 9 → 10), and
`.specify/tools/{system,shell,project}.json` were regenerated through `generate-instructions.sh`'s
own redirections, with the new engine present in `project.json` (6 occurrences). `tools.md` was
**deliberately not hand-edited**: it is not a derived manifest of this kind, and adding a
project-script entry to an MCP index would be wrong content in the wrong file. DoD-8's `tools.md`
clause is **void** (unsatisfiable as written), not skipped or deferred.

**Class-level finding (out of scope for feature 050, reported not fixed)**: the same false claim is
repeated in **seven surfaces** — `templates/instructions-template.md:132`, `templates/tools.md:14`,
`templates/commands/tools.md:103`, `shared/definitions/tool-definitions.md:36`,
`skills/create-tools/SKILL.md:65`, `skills/improve-tools/SKILL.md:53`, and the generated
`.specify/instructions.md:147`. Because the first is ambient, every agent inherits it. This is
Principle XIV's rule of thumb failing loudly: correcting one fact requires editing six framework
files. The fix belongs to the owner (`shared/definitions/tool-definitions.md`) plus a pointer-shaped
sweep of the other five, and should ride its own change rather than this feature's commits.
