...
| 决策 | 结果 | 规格改动 |
|------|------|----------|
| Feature 绑定 | 037 Docs Command(第四次 follow-up),`Total Features` 保持 48 | `Related Feature` 解析 + 绑定依据;`features/037.md` Related Specifications 交叉引用;features.md 037 行追加 follow-up 说明 |
...
后置校验:残留占位符 0;历史 Clarifications 追加式保留;FR 计数 19(FR-001…FR-018 + FR-002a),SC 计数 7;实现期用户修订“写什么/放哪里 + 固定检索入口”已映射到 FR-016…FR-018 / SC-007;标题层级完好;`Total Features` 未变。
