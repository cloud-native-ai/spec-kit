...
6. **Tool manifests**
   - Run `.specify/scripts/bash/create-new-skill.sh --refresh-only --name <name> --json` to regenerate `${SKILL_HOME}/tools/{system,shell,project}.json` and ensure `skill_id` exists in the frontmatter.
