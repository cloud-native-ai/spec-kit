...
10. **Register `tool_id` in instructions template**
   - After a tool is selected or created, add one structured list entry to the `## Resource Registry` → `### Tools` subsection in `.specify/instructions.md`, using field names aligned with the selected `.specify/templates/tool-*-template.md` file.
