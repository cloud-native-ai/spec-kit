...
You **MUST** analyze the user input in `$ARGUMENTS`, infer the user's intent, and use that intent to supplement TODO collection and insertion while preserving the default workflow.
...
3. If `$ARGUMENTS` is empty, still generate a complete reviewable TODO plan from available scanner output.
...
1. **Background Info Type**: If `$ARGUMENTS` primarily contains descriptive text, business context, constraints, or unstructured information
   - Integrate this information as context into the plan generation process
