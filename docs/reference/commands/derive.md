...
- **Concept authority**: `.specify/shared/definitions/derivation-definitions.md` — the four record schemas, the provenance grades, the chain-integrity rules C1–C7, the self-audit set A1–A14, the banned-justification set and the capability-degradation rule are defined there. This page references them and restates none of them.
...
7. **Self-audit** — `validate` decides the mechanical half of the audit set and returns `errors[]`, `warnings[]` and `semanticChecksPending`; exit code 4 means a structural violation. `semanticChecksPending` is always `["A11", "A14"]` — *online grounding actually happened this run* and *the artifact states the sources' way of thinking rather than a summary of their content* — and those two must be **attested by the agent**, never inherited as green.

   Because `validate` is zero-write, the audit is a **two-pass loop**, not one call. A1–A10/A12/A13 are *derived* from `errors[]`, and the artifact's own `## Self-Audit` rows must equal those derived values or the engine reports `audit-result-diverges`. A freshly scaffolded archive carries `pending` in all fourteen rows and therefore does **not** validate clean. So: run `validate` once, transcribe `payload.audit.engine` into the A1–A10/A12/A13 rows, set A11 and A14 to `attested` with a `method` sentence a reader could actually check (an empty, `engine` or `n/a` method draws the `attestation-method-degenerate` warning), then run `validate` again to converge. Transcribing the results into the artifact is the agent's step, never the engine's.
