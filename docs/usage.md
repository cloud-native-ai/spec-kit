...
- **Deterministic ID**: 每个 skill 会生成并持久化 `skill_id`（默认是 `.github/skills/<name>/SKILL.md` 的工作区相对路径）
...
### `speckit.tools`
**Purpose**: Define/find reusable tools from MCP/System/Shell/Project sources with persistent records, then invoke only after explicit confirmation.
