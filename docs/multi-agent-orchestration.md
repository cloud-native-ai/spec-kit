...
| **Team Loop** | Supervisor + Coordinator + Workers → Iterative quality loop | Quality-critical deliverables needing refinement |
...
1. **Coordinator** decomposes the feature into two independent modules.
...
5. **Aggregation**: Coordinator collects results, runs integration tests, produces unified report.
...
A Team Loop requires three role types:

| Role | Responsibility | Example Agents |
|------|---------------|----------------|
| **Supervisor** | Quality gating, convergence detection, iteration control | team-supervisor |
| **Coordinator** | Task decomposition, worker dispatch, result collection | meta-coordinator |
| **Workers** | Execute specific sub-tasks within their expertise | module-designer, test-engineer |

The Supervisor sits at the top, evaluating team output each iteration. The Coordinator dispatches work to Workers. Workers produce artifacts that flow back to the Supervisor for scoring.
...
Iteration 2:
  Improver (Worker) → addresses feedback, rewrites spec
...
Iteration 3:
  Improver (Worker) → final refinements
...
- **Coordinator** (dispatch, aggregation): Lightweight — mostly structural work
- **Workers** (implementation, testing): Standard — requires coding ability
- **Supervisor** (quality scoring, convergence): Standard to High — requires judgment
- **Evaluator** (rubric scoring): Standard — structured evaluation
- **Improver** (rewriting based on feedback): Standard — targeted modifications
...
- Improver should only rewrite low-scoring sections, not the entire artifact
...
| Score oscillates without improving | Improver and Evaluator disagree on quality | Align scoring rubric with improvement instructions |
