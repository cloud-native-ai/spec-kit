# EEI Triad Pattern: Executor-Evaluator-Optimizer
...
- **Role-scoped triad**: the supervisor spawns Executor/Evaluator/Optimizer subagents whose task, environment paths, and default scoring dimensions are bound to the role's domain via the orchestration template's `{{ROLE_SCOPE}}` binding.
