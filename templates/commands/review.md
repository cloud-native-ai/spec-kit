...
1. **Resolve feature context**
   1. Run `{SCRIPT}` from repo root and parse FEATURE_DIR, FEATURE_ID, FEATURE_NAME, and AVAILABLE_DOCS. All paths must be absolute. For single quotes in args like "I'm Groot", use escape syntax: e.g. `'I'\''m Groot'` (or double-quote if possible: `"I'm Groot"`).
   ...
   3. Determine the corresponding feature memory file path:
      ...
      - If neither is available, create a new memory file at `.specify/memory/features/[FEATURE_KEY].md` using `templates/feature-details-template.md` as the base structure.
...
6. **Update the feature memory file** in `.specify/memory/features/`:
   1. Load the existing feature memory file (or instantiate from `templates/feature-details-template.md` if creating it for the first time).
