...
1. **Resolve feature context**
   ...
   3. Determine the corresponding feature memory file path:
      ...
      - If neither is available, create a new memory file at `.specify/memory/features/[FEATURE_KEY].md` using `templates/feature-template.md` as the base structure.
...
6. **Update the feature memory file** in `.specify/memory/features/`:
   1. Load the existing feature memory file (or instantiate from `templates/feature-template.md` if creating it for the first time).
