...
Run `{SCRIPT}`; parse REQUIREMENTS_DIR, FEATURE_ID, FEATURE_NAME, AVAILABLE_DOCS. Then capture via git/shell: REPO_NAME, REPO_URL, BRANCH, COMMIT_SHA, REPO_ROOT_ABS, REVIEW_DATE, REVIEWER, ENVIRONMENT, SPECKIT version, ARTIFACT_INVENTORY (basename, path, line count, one-line summary per artifact).
...
Use `templates/review-template.md` structure. Fill all sections: Context → Timeline → Findings Summary → Findings → What Worked (brief bullets only) → Recommendations (cite target files) → Priority Roadmap. Self-containment check before writing. Write to `REQUIREMENTS_DIR/review.md`.
...
At wrap-up (the same lifecycle point where this command prompts for a Git commit), perform an agent self-reflection step (never solicit feedback content from the user), following the canonical convention in `.specify/skills/sdd-workflow/references/feedback-step.md`:
...
**After**: Apply improvements to spec-kit templates/commands/scripts. Optionally iterate requirements/plan.
