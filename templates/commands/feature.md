---
description: feature command主要实现三个功能：无参数时进行全局feature生成/补充并更新memory/features.md与memory/features/<ID>.md；传入具体信息（如git commit id）时基于该信息发掘/补充feature；传入描述但不含具体信息时在memory/features.md中定位目标feature并基于最新状态更新。
handoffs: 
...
2. Feature Index: `memory/features.md` generated from the installed template at `.specify/templates/features-template.md` (source development template: `templates/features-template.md`).
...
3. **根据模式执行 Feature 更新**
    ...
    - 仅描述参数（不含具体信息）：
       - 在 `memory/features.md` 中定位可能需要更新的 Feature。
...
10. **同步更新项目根目录 README 的特性列表**：
   - 读取 `memory/features.md` 的表格内容并进行整理。
...
- 在执行 `/speckit.specify`、`/speckit.plan`、`/speckit.tasks`、`/speckit.implement` 后，都应主动同步更新 `memory/features/*.md` 与 `memory/features.md`（若有变化）。
