...
- **Warning (continue with fallback)**:
   - `{SCRIPT}` exits non-zero but required directories/files already exist.
...
2. **Establish the Refresh Base + Observation Snapshot** (skip entirely if no `.specify/instructions.md` existed before this run):
   ...
   - **Inventory sections (mandatory artifact — observation snapshot)**: take the sizes and the two classifications **from Action 1's section table**, never by re-measuring the file by eye. The script already compared every top-level section of the base against the rendered template and emitted one row per section — `section | live B | tmpl B | delta | placeholder | project-owned`. Those columns are the machine's verdict: consume them as-is, and add exactly one column of your own — whether the section is auto-derivable from a codebase scan (e.g., raw Tech Stack facts, Documentation Map paths) or hand-authored / non-reproducible (custom governance rules, recurring lessons, decision rationale). Hand-authored sections are must-keep and are only touched to correct a clearly stale fact. This table-plus-one-column is the diff baseline for Action 5, and Routes R1/R2 draw their candidates **only** from it — a section with no row is not a candidate, and a candidate's size and marks are read off its row rather than argued.
...
3. **Recover content lost to older overwriting versions** (run whenever any `.specify/instructions.md-*` backup exists; this is the repair path for projects damaged before the non-destructive fix):
   ...
   - **Backup retention rule**: the keep-set is the **newest 5** backups plus every backup that still holds a `## ` heading absent from the live file — that heading is unrecovered content, so such a backup stays load-bearing however old it is. Name every backup **outside** the keep-set in the residual report (Action 7) as the prunable set; naming them is what stops `.specify/instructions.md-*` from growing invisibly across runs. This command never removes a backup itself — pruning is the user's call on that list (Archive-not-delete, **Update Strategy** above).
...
5. **Section-by-section refresh** (the diff-and-converge pass — operate directly on the base file; a newly created file starts empty-of-user-content, so its sections are simply filled in):
   ...
   - **Spec Kit Framework Map**: keep this section in the template's **summary + pointer** shape (short highlights paragraph linking `shared/definitions/framework-map.md`) — never expand it back into an inline table or manual. Verify the link target exists; add genuinely new framework locations to the external doc, not to this section. If prose here starts explaining procedures, compress it back to a pointer.
   - **Size-budget convergence (triggered by the setup script's report)**: Action 1's script run prints the file's byte size against its budget and, on every run, the section table Action 2 turns into the observation snapshot. That report is this step's trigger — treat "budget reached" as a convergence condition in its own right, not as a cosmetic warning to relay. The budget value is owned by the script (`INSTRUCTIONS_BUDGET_BYTES` in `{SCRIPT}`); read it from the report and never restate the number here. Both routes below read their candidates and their sizes off the snapshot's rows; neither re-measures the file. Two routes, applied largest-section-first until the report comes back within budget:
     - **Route R1 — a template-owned section has drifted thicker than the template's own shape.** **Filter before reading any prose**: drop every snapshot row marked `placeholder=yes`, every row marked `project-owned=yes`, and every row whose `delta` is not positive. Both marks are machine-checked columns of the script's table — the placeholder discriminator is a `[`…`]` or `{{VAR}}` token surviving in that section's *rendered template* version — so the filter is a lookup on the row, never a judgment call. What survives, largest `delta` first, is R1's entire candidate pool. Each surviving row is a template-owned section whose live version *may* have accumulated inline detail the template deliberately keeps out; compress it back to the template's summary + pointer form. Two filters decide whether it actually may, and skipping either one turns R1 into deletion:
       - **Exclude project-filled sections first.** A section whose template version still carries a placeholder token (`[Bracketed ...]`, `{{VAR}}`) is a *scaffold the project is supposed to fill*, so live-larger-than-template is its correct end state, not drift — `## Documentation Map`, `## Tech Stack & Resources` and `## Project Overview` are all of this kind, and on a mature project they are among the largest sections in the file. Compressing one back to the skeleton deletes real project content. That is why the exclusion above is the route's opening act rather than a check applied to a shortlist somebody already picked.
...
7. **Report (mandatory artifact — residual report)**:
   ...
   - **Name the prunable backups**: list every `.specify/instructions.md-*` backup that Action 3's retention rule placed outside the keep-set. The rule itself is owned by Action 3; this bullet only carries its output into the report, so the history's growth stays visible to the user rather than accumulating silently.
