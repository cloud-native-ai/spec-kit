...
5. **Section-by-section refresh** (operate directly on the base file — a newly created file starts empty-of-user-content, so its sections are simply filled in):
...
7. **Report**:
   ...
   - Summarize which sections were left untouched, which had stale facts updated, which (if any) new template sections were added, and any content recovered from backup history.
