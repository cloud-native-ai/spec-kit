...
2. **Frontmatter fields** (in this order)
   ...
   - `skill_id`: `<SKILL:.specify/skills/<name>/SKILL.md>`. If missing, regenerate via `scripts/bash/create-new-skill.sh --refresh-only --name <name> --json`.
...
3. **Path conventions — `${SKILL_HOME}` / `${SKILL_WORKDIR}`**
   ...
   - For each shell script under `${SKILL_HOME}/scripts/`, ensure the FR-016 idiom appears at the top:
     ```bash
     SKILL_HOME="${SKILL_HOME:-$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")/.." && pwd -P)}"
     SKILL_WORKDIR="${SKILL_WORKDIR:-$(pwd -P)}"
     ```
   - Scripts nested deeper than `${SKILL_HOME}/scripts/<name>.sh` MUST adjust the `..` count accordingly.
...
4. **Migration Mapping (legacy idioms → new form)**
   ...
   - Bare relative paths (`./scripts/init.sh`, `./references/spec.md`) → `${SKILL_HOME}/scripts/init.sh`, `${SKILL_HOME}/references/spec.md`.
...
- The `SKILL_HOME` fallback assumes the conventional placement `${SKILL_HOME}/scripts/<name>.sh`. Scripts nested deeper (e.g., `${SKILL_HOME}/scripts/sub/<name>.sh`) must adjust the `..` count accordingly.
...
|--------------|----------|----------------|---------------|
| Bare relative path (`./X`) | `${SKILL_HOME}/X` | `./scripts/init.sh` | `${SKILL_HOME}/scripts/init.sh` |
...
- **Script execution**: Use `Bash` tool to run `scripts/bash/create-new-skill.sh`. Check exit code explicitly — the script may exit 0 even on partial failure.
