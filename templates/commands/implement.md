...
1. Run `{SCRIPT}` from repo root; parse REQUIREMENTS_DIR and AVAILABLE_DOCS. All paths must be absolute.
...
7. **Progress tracking**:
   ...
   - Mark completed: `[X]`. Deferred: `[~]` with `<!-- deferred: <reason> -->` — the reason domain covers BOTH a resource unavailable this round AND an exhaustive requirement that cannot be completed this round; a `[~]` is an honest non-green, not a state to hide. Never leave deferred work as `[ ]`. When a `[~]` makes the Completion Gate (step 8) fail, that fail is an EXPECTED outcome — it MUST NOT be turned green by rewriting the task description or relaxing the gate criteria (the task-side disposition forms are owned by `templates/commands/tasks.md`).
...
8. **Completion validation**: All tasks `[X]` or `[~]` (no `[ ]` remaining). Features match spec. Tests pass. **Completion Gate re-validation**: if tasks.md contains a `## Completion Gate` section, do NOT trust the all-tasks-done state — re-validate every gate item against the current tree (running its stated check), and treat any failing item as an open task. Track consecutive gate rejections: after **3** consecutive failed re-validations without a newly closed item, STOP and escalate to the user instead of retrying. This counter has **run scope** — it resets to zero at the start of every run (it is never a cross-run total), and its current value is written into `verification.md` so a resumed run reads the real count instead of inheriting a stale one. **Gate-premise decay** (named action): when a premise a gate item or DoD row rests on — a frozen SHA, a line number, a count — is falsified by measurement, re-reference the premise, measure the new form **more strictly, never more loosely**, and annotate the falsified premise inline on that row; a gate item or DoD row whose premise is measurably false is **void**, not unmet, and the void rule with its surfacing duty is owned by `shared/workflow/feature-integration.md` § Pre-Status-Flip Gate (which binds `## Completion Gate` items exactly as it binds DoD rows). **Commit gate**: commit after each task or logical group; in multi-phase runs the **phase boundary** is the default commit unit — commit only after that phase's name-level regression diff (`comm -13 baseline current`) is empty, so every increment is independently bisectable; count-only regression claims are not acceptable evidence. **Legitimate-transient exception** (owner: `docs/reference/history/00-cross-cutting-lessons.md` §二十二): when an existing guard goes red purely because of task ORDERING — a discipline truth-source lands in one phase while its reachability pointer lands in the next — the red is a legitimate transient, not a regression. Do NOT commit a known-red increment; combine the two phases into ONE commit unit whose criterion is that the MERGED `comm -13 baseline current` is empty, and attribute it explicitly as an ordering transient (neither a subject defect nor an assertion defect). **Approval routing**: each phase-boundary commit goes through ONE explicit approval point — present the phase's commit command and wait for the user's go-ahead, UNLESS the user pre-authorized commits for this run (e.g. "直接提交" / long-run mode), in which case commit without re-asking. A run MUST NOT end with the feature entirely uncommitted while commit commands were never presented: if every approval point was declined or skipped, the wrap-up MUST prominently list the pending uncommitted state and the exact commit command(s) as the final actionable item. **Deletion-surface audit (mechanical, before every phase commit)**: run `git diff --cached --diff-filter=D --name-only` (and the same for the unstaged tree before staging) — every listed deletion MUST reconcile against a task row or a user-confirmed cleanup plan; an unreconciled deletion aborts the commit for human review (`git add -A` silently carries working-tree destruction into otherwise-green commits — this audit is the backstop). The spec dir MUST NOT be left *entirely* uncommitted when validation completes — an uncommitted implementation leaves no per-task audit trail and breaks `/speckit.review`'s git-based history reconstruction. Do not report the Definition of Done as "met" while the whole feature is uncommitted.

9. **Pre-Status-Flip Gate** and **Verification Log**: Apply the full gate protocol from `shared/workflow/feature-integration.md` § Pre-Status-Flip Gate. Populate `REQUIREMENTS_DIR/verification.md` from `.specify/templates/verification-log-template.md` — that template OWNS the `SC-NNN_status` vocabulary, so draw every status from the set it declares and MUST NOT narrow it to a subset (a gate or DoD row reading `pass|deferred` silently forbids an honest `partial`).
...
Fallback for runs whose phase-boundary commits were declined or skipped (see the Commit gate's approval routing in step 8). After implementation, generate commit command using `templates/commit-template.md`:
