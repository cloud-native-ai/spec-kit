...
1. 从仓库根目录运行实现前置检查脚本 `{SCRIPT}`（当前为 `scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks`），并从其 **JSON 输出中解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表**。具体约定：
   ...
   - FEATURE_DIR 由 `scripts/bash/common.sh` / `check-prerequisites.sh` 内的逻辑根据当前 Git 分支或目录结构解析（遵循 `.specify/specs/###-feature-name/` 约定）；**不得在 Agent 层面凭空构造或猜测**。
