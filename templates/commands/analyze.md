...
handoffs:
  - label: Update Feature Registry
    ...
    prompt: Update feature metadata to resolve linkage or consistency findings from analysis.
    send: true
...
Process `$ARGUMENTS` per the [User Input Protocol](shared/workflow/user-input-protocol.md). Treat as supplemental analysis focus areas, not standalone instructions.
...
Run `{SCRIPT}` once from repo root and parse JSON for REQUIREMENTS_DIR and AVAILABLE_DOCS. Derive absolute paths:
...
**Same-author detection delegation (mandatory when the artifacts are self-authored)**: when the artifacts under analysis were produced **by the agent now running this command, in the current session** — the ordinary case for a `requirements → clarify → plan → tasks` chain run without a break — self-review is weak evidence, because the author's own misreading is reproduced in the audit: both passes read the text the same way. Detection MUST then be delegated to fresh-context read-only subagents, not merely the §5.5 validation wave. Record in the report that this condition held and how detection was split.

- **Split by disjoint artifact scope**, one subagent per scope, so no two agents audit the same file pair and none needs the whole artifact set in context. A three-way split that has held up: (a) spec ↔ plan ↔ research consistency — duplication, ambiguity, underspecification, count and cross-reference drift; (b) plan ↔ contracts ↔ data-model ↔ tasks coverage — every requirement reaches a task, every task maps back, every claimed mapping is true; (c) feature linkage + registry + constitution alignment.
- **Name the same-author condition in the brief.** A subagent that does not know the artifacts are self-authored will not hunt the defect class that dominates them: a correction applied in one place and not in the others. State that condition and the multi-pass editing history, and instruct the agent to assume defects exist and locate them — not to confirm the artifacts are sound.
- **Require a propagation-surface answer before any severity.** For every finding the detector MUST list which downstream artifacts inherit it; when the answer is none, severity is capped at MEDIUM. This is the cheapest available correction: in the run that established this rule, 12 findings reached validation at CRITICAL/HIGH and **10 were downgraded**, and every downgrade reason was a propagation fact the detector had not checked — the defect never reached a downstream artifact, an existing test already guarded it, a disclaimer was misread as a claim, or an attribution was wrong while the coverage existed. Asking at detection time moves that work upstream and shrinks the validation wave.
- **Treat every mapping table as an assertion, never as evidence.** An FR → clause → task table can be fully self-consistent — both identifiers resolve, the counts agree — while the referenced clause says something else entirely. Checking that a target *exists* is not checking that it *expresses* the source. Detection MUST quote the referenced clause body and judge whether it carries the mapped obligation; this defect class is invisible to any ID-level cross-check.
- **Subagent-unavailable fallback**: as §5.5 — if dispatch fails twice in a row (upstream error), degrade to a direct bounded read and mark every resulting row `(detected: direct read, subagent unavailable)` so the weaker evidence path stays visible to the reader.
...
- **Batching**: validate findings in one parallel dispatch wave; a validator MUST NOT validate a finding it produced. When detection was also delegated under §4's same-author rule, the detection agents and the validators MUST be disjoint sets — a detector validating its own finding is the self-review this pass exists to remove.
...
- Self-authored artifacts are not self-auditable: when this session produced them, delegate detection as well as validation (§4 **Same-author detection delegation**)
