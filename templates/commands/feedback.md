---
description: Single entry point for feedback operations — the command judges where the accumulated feedback points and routes itself, with no keyword to remember. Feedback about this project is digested in place (Path A, whose analysis step is the introspection 自省); feedback about the framework upstream is packaged for manual delivery (Path B). Injecting a probe for a client-project custom unit stays an explicitly-requested capability, because a target unit id cannot be inferred from repo state.
...
- **Fix at the owning hat**: in the framework repo, findings are acted on in the framework source (`templates/`, `skills/`, `scripts/`, `shared/`, `src/`), never in `.specify/` mirrors (two-hats rule: Constitution XI). In a client project, findings about a custom unit are acted on in that unit's own files.
