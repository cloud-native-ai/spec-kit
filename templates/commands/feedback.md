---
description: Single entry point for feedback operations — the command judges where the accumulated feedback points and routes itself, with no keyword to remember. The hat decides first: in the framework project every entry points at this project and is digested in place (Path A), because this repo is the upstream and a packaged zip would be addressed to itself; in a client project, entries about its own custom units are digested (Path A) and entries about framework units are packaged for manual delivery upstream (Path B). Introspection(自省) is a step of the digest path, not a separate invocation. Injecting a probe for a custom unit stays an explicitly-requested capability, because a target unit id cannot be inferred from repo state.
short-description: 反馈统一入口：按指向自动分流为就地消化或打包上送
---
...
- **Interrupted-run recognition (the mirror case of the cross-check above)**: a bundle in the intake with **no** matching `Bundles` row is not automatically new input either — it is equally what an interrupted Path A run leaves behind, because the consume-log row is that run's only persistent marker and A5 writes it **last**. Put such a bundle to the user as **one** question — is this a **已确认待清理** resumption of an earlier run, or genuinely new input? — and do NOT re-route it as new input by default. Record the answer in that batch's consume-log row so the next run reads a fact instead of facing the same ambiguity. A resumption picks A5 back up (its bundles were already confirmed and are awaiting removal); new input continues from A2.
...
- **Uninterrupted sequence**: preserve (orphans) → remove the bundles → append the consume-log row MUST run as **one uninterrupted sequence**, with no other work inserted between its steps — not another finding, not a routed `improve-*` run, not a wrap-up step. The reason is structural, not stylistic: the log row is this batch's **only** persistent marker and it is the sequence's *last* step, so an interruption anywhere before it leaves removed bundles with no record that they were ever routed — which the next run cannot tell apart from a batch nobody processed. That indistinguishability is exactly what A1's interrupted-run recognition has to ask the user about, so an interruption here manufactures the ambiguity there.
...
5. **Post-package cleanup (hat-aware closing step of the package run)**: whether the packaged batch may leave the active store depends on which hat this repo wears — decided by the Routing flow's step-2 criterion, not restated here.
   - **Client project (the default)**: once the zip exists, the packaged batch no longer needs to live in the active store — the zip is on its way to a recipient that can act on it, so the zip is the record. Preview with `--action cleanup --package <zip|latest> --dry-run`, then run without `--dry-run` in the same session as packaging. Cleanup removes only entries actually inside that zip; `cleanup-log.md` records every removal. The zip itself STAYS under `.specify/memory/feedback/packages/` as the delivery artifact.
   - **Framework project**: run cleanup **only when the user's request for this same run also asked for `mark-submitted`** — that pairing is what states the batch is genuinely settled. Without a recipient (see the paragraph above), emptying the active store would leave the entries' only actionable form inside a zip nobody can deliver, and the store would no longer be able to answer `--action list`. When the pairing is absent, **keep the entries in the store** and list them in the report as **待定 (pending)**: packaged, undelivered, still actionable, deliberately not cleaned up.
