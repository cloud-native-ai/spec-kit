...
2. **Check for existing branches before creating new one**:
   ...
   d. Prepare to run the script `{SCRIPT}` with the calculated number and short-name.
   
2. Run the script `{SCRIPT}` from repo root. Parse its JSON output for BRANCH_NAME and SPEC_FILE. All file paths must be absolute.

   **IMPORTANT**:

   - For Bash, this expands to a heredoc-based, safe JSON handoff that writes the raw user input to stdin and passes its contents to `{SCRIPT}`. This avoids shell parsing issues with quotes, backslashes, and newlines.
...
2. **Bind or Create**:
   ...
   - **If NO matching Feature is found**:
     - Create a new Feature:
       ...
       - Add a new row to `memory/feature-index.md`.
