...
本命令是推导能力的执行入口。**概念真源** = `.specify/shared/definitions/derivation-definitions.md` — 四套记录 schema(Source / Reasoning Move / Derivation Step / Architecture Element)、四级溯源等级、链完整性规则 C1–C7、自审集 A1–A14、封闭禁用论证字面量集、能力降级规则均在该文件定义。本模板**引用而不复述**其规则;二者不一致时以概念真源为准。引擎 = `.specify/scripts/python/derive-utils.py`(动作与退出码以其 `--help` 为准)。
...
- **引用不复述**:schema、等级集、C1–C7、A1–A14、禁用字面量集一律以 `.specify/shared/definitions/derivation-definitions.md` 为准。
