...
- **Failing-test baseline name list** — its path, its capture flag, and its name-level comparison form are owned by `/speckit.implement` step 6's `**Test runs**` bullet (`.specify/templates/commands/implement.md`). MUST NOT be restated, re-derived, or given a second path here; a Setup/Foundational row that captures or consumes that baseline points at that owner.
