...
def _probe_head_commit(start_dir):
    ...
    is data. The build hook (hatch_build.py) reuses this same function; a
    second probe implementation must not exist."""
    try:
        proc = subprocess.run(
...
def _read_embedded_source_commit():
    """Read the build-embedded source commit next to this module. The file is
    a build artifact with untrusted content: any read/parse/shape failure is
    'no embedding', never an exception (043 contract §C-3)."""
    try:
        payload = json.loads(
            (MODULE_DIR / _SOURCE_COMMIT_FILENAME).read_text(encoding="utf-8"))
...
def resolve_source_commit():
    ...
    embedded = _read_embedded_source_commit()
